...
from src.apps.recommendation.lib.recommender_als import (
    RecommenderALS,
)
...
class TritonPythonModel:
    ...
    def initialize(self, args):
        ...
        self.post_ids_dtype = pb_utils.triton_string_to_numpy(post_ids_config["data_type"])
    ...
    def convert_model_response_to_triton_response(self, model_responses, strategy_dict):
        post_ids = pd.DataFrame([recommendation.item_id for recommendation in model_responses])
        scores = pd.DataFrame([recommendation.score for recommendation in model_responses])
        data = pd.DataFrame([strategy_dict["strategy"]])

        # Create output tensors. You need pb_utils.Tensor
        # objects to create pb_utils.InferenceResponse.
        post_ids_tensor = pb_utils.Tensor("post_ids", post_ids.values.astype(self.post_ids_dtype))
        scores_tensor = pb_utils.Tensor("scores", scores.values.astype(self.scores_dtype))
        strategy_tensor = pb_utils.Tensor("strategy", data.values.astype(self.strategy_dtype))
    ...
    def recommend(self, profile_id, top_n, model_type):
        ...
        inference_response = self.convert_model_response_to_triton_response(recommendations, strategy_dict)
        return inference_response

    def look_alike_items(self, post_id, top_n):
        recommendations, strategy_dict = self.model.look_alike_items(item_id=post_id, top_n=top_n)
        inference_response = self.convert_model_response_to_triton_response(recommendations, strategy_dict)
        return inference_response
    ...
    def execute(self, requests):
        ...
        for request in requests:
            ...
            model_type = pb_utils.get_input_tensor_by_name(request, "model_type").as_numpy()[0].decode("utf-8")

            if method == "recommend":
                profile_id = pb_utils.get_input_tensor_by_name(request, "profile_id").as_numpy()[0].decode("utf-8")
                inference_response = self.recommend(profile_id, limit, model_type)
                responses.append(inference_response)

            elif method == "look_alike_items":
                post_id = pb_utils.get_input_tensor_by_name(request, "post_id").as_numpy()[0].decode("utf-8")
